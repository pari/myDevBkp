...
// --useconfig=FILE -sh=HOST -su=USER -sp=PASSWORD {-dh=HOST -du=USER -dp=PASSWORD} {-opf=op.txt} --locktables='Y'
	...
	foreach( $config as $dbname => $tables  ){
		...
		$fetch_views_cmd = "mysql -u $su -p{$sp} -h {$sh} INFORMATION_SCHEMA --skip-column-names --batch -e \"select table_name from tables where table_type = 'VIEW' and table_schema = '{$dbname}'\"  | xargs -r mysqldump -u {$su} -p{$sp} -h {$sh} --skip-add-locks {$dbname}" ;
		if($opf){
